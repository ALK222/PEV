...
public class IndividuoFuncion1 extends Individuo<Boolean> {
	...
	public IndividuoFuncion1() {
		...
		for(int i = 0; i < tamTotal; i++) this._chromosome[i] = this.rand.nextBoolean();
	}
	...
	public double getFenotipo(int index) {
		...
		if(index == 0) start = 0;
		else start = this.tamGenes[index-1];
		...
		return this.min[index] + (res * (this.max[index] - this.min[index]) / (Math.pow(2, this.tamGenes[index] - 1)));
	}
